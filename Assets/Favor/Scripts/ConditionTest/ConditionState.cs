...
namespace FavorSample
{
    ...
    public class SampleStateMachine
    {
        ...
        Dictionary<Enum, SampleBaseState<SamplePlayer>> dict;
        ...
        // 상태 변화 함수. 조건 상태가 아닐 경우 조건상태에 필요한 변수는 default 매개변수로 선언
        public void ChangeState(Enum state, bool cond = true, Action act = null)
        {
            SampleBaseState<SamplePlayer> next;
            // 만약 조건 상태로 변경할 경우
            if (state is SampleStates.Condition)
            {
                // 조건을 초기화
                (dict[state] as ConditionState).InitCondition(cond, act);
            }
            next = dict[state];
    ...
    public class ConditionState : SampleBaseState<SamplePlayer>
    {
        ...
        // 조건 초기화 함수
        public void InitCondition(bool cond, Action act)
        {
            this.cond = cond;
            this.act = act;
        }
    ...
    public class CheckState : SampleBaseState<SamplePlayer> 
    {
        ...
        public override void OnEnterState()
        {
            // 코드블럭 세팅
            blocks.Clear();
            blocks = BlockContainerManager.Instance.GetContatinerBlocks();
    ...
    public class SampleStateChanger
    {
        ...
        public void ConditionTest(Vector2Int pos)
        {
            ...
            bool cond = StageManager.Instance.CheckMonsterAndPlayerPos(pos);
            // 조건 만족시 수행할 행동을 설정
            Action act = ChangeStateToBattle;

            // 조건 수립 후 상태 변경
            player.StateMachine.ChangeState(SampleStates.Condition, cond, act);
        }
